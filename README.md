...
## News
- 2024.03.09 - v0.0.4. New params: `--stop-words` ("", list for llama separated by semicolon: `;`), `--min-tokens` (0, min tokens to output), `--split-after` (0, split first sentence after N tokens for xtts), `--seqrep` (false, detect loops: 20 symbols in 300 last symbols), `--xtts-intro` (echo random Umm/Well/...  to xtts right after user input)
